...
float calcularPuntaje(struct un_jugador *jugador)
{
    ...
    float w_V = 0.6;   // Velocidad
    float w_C = 1.0;   // Control del balón
    float w_D = 0.8;   // Defensa
    float w_AT = 0.9;   // Ataque
    float w_G = 0.5;   // Gambeta
    //float w_AG = 0.5;   // Agresividad
    float w_P = 0.6;   // Porteria
    float w_VJ = 0.75;  // Visión de juego
    float w_JE = 0.6;  // Juego en equipo
    float w_E = 0.55;   // Experiencia
    */
    float w_R = 0.75;   // Resistencia
    ...
    // puntaje += w_AG * jugador->agresividad;
...
void cargarListaDeJugadores(FILE* archivo, struct un_jugador *jugadores)
{
    ...
    while (fgets(buffer, MAX_LENGTH, archivo))
    {
        ...
        //jugador.agresividad = conseguirSiguienteFloat(buffer, &inicio, &fin);
        ...
        //jugador.experiencia = conseguirSiguienteFloat(buffer, &inicio, &fin);
...
int main(void) {
    ...
    struct un_jugador *jugadores_disponibles[tam_equipo * 2];
    int indice_jugador = 0;
    int contador = 0;
    
    moveTo(3, 39);
    printf("Elija %d jugadores: ", tam_equipo * 2 - contador);
    scanf("%d", &indice_jugador);
    while(contador < tam_equipo*2 && indice_jugador > -1)
    {
